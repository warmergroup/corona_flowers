...
<template>
    ...
    <div v-else class="p-3">
        <div class="space-y-4">
            <div
                class="p-4 border rounded-lg shadow-md flex items-start space-x-4 dark:bg-gray-800 dark:border-none bg-white">
                <div class="w-24 h-24">
                ...
                <div class="my-auto flex gap-2">
                    <UButton @click="onEdit" :disabled="!isOwner" label="Tahrirlash" color="yellow"
                        ...
                        :title="isOwner ? '' : `Faqat post egasi o'zgartirishi mumkin`" />
                    <UButton @click="onDelete" :disabled="!isOwner" label="O'chirish" color="red"
                        icon="material-symbols:delete-outline"
                        :title="isOwner ? '' : `Faqat post egasi o'chirishi mumkin`" />
                </div>
            </div>
        </div>
    </div>
